...
dset = Variable.get("factloadjob2", deserialize_json=True)
...
with DAG(dset["name"], default_args=default_args, schedule_interval=None, dagrun_timeout=timedelta(minutes=dset['dagrun_timeout'])) as dag:
        ...
        with TaskGroup(group_id="{}_SqooptoFact".format(stagegrp)) as run_stage0:
            ...
            with TaskGroup(group_id="{}_depstagetab".format(stagegrp)) as run_depstage:
                ...
                for stagedeptab in sqoopjobs[stagegrp]["depstage"]:
                    ...
                    command="base64 -d <<< {} | kinit {}@{} && ssh -o StrictHostKeyChecking=no -o GSSAPIAuthentication=yes -o GSSAPIDelegateCredentials=yes {}@{} '{}'".format(password, kinitprincipal, kinitdomain, kinitprincipal, edgenodehost,"{} {} {} {} {} {}".format(scriptpaths["hiveload"], tabname , batchid,  'dml', dbname, 'stage'))
            ...
            with TaskGroup(group_id="{}_FactLoad".format(stagegrp)) as run_fact:
                ...
                for facttab in sqoopjobs[stagegrp]["facttabs"]:
                    ...
                    command="base64 -d <<< {} | kinit {}@{} && ssh -o StrictHostKeyChecking=no -o GSSAPIAuthentication=yes -o GSSAPIDelegateCredentials=yes {}@{} '{}'".format(password, kinitprincipal, kinitdomain, kinitprincipal, edgenodehost,"{} {} {} {} {} {}".format(scriptpaths["hiveload"], tabname , batchid,  'dml', dbname, 'fact'))
            ...
            with TaskGroup(group_id="{}_DepFactLoad".format(stagegrp)) as run_depfact:
                ...
                for dep in sqoopjobs[stagegrp]["depfact"]:
                    dbname, tabname = dep.split('.')
                    command="base64 -d <<< {} | kinit {}@{} && ssh -o StrictHostKeyChecking=no -o GSSAPIAuthentication=yes -o GSSAPIDelegateCredentials=yes {}@{} '{}'".format(password, kinitprincipal, kinitdomain, kinitprincipal, edgenodehost,"{} {} {} {} {} {}".format(scriptpaths["hiveload"], tabname , batchid,  'dml', dbname, 'fact'))
                    taskname = "{}".format(tabname)
                    ssh_fact = getbashoperator(taskname, False, command)
    ...
    dummyop1 = DummyOperator(task_id='NoOPFact2')
