...
from airflowcommon import getBatchId, getpythonoperator, getbashoperator, getsshoperator
        ...
        with TaskGroup(group_id="{}_SqooptoFact".format(stagegrp)) as run_stage0:
            ...
            with TaskGroup(group_id="{}_SqooptoStage".format(stagegrp)) as run_stage1:  
                ...
                for landtab in sqoopjobs[stagegrp]["landtab"]:
                    ...
                    command='{} {} {}'.format(scriptpaths["sqoop"], tabname, dbname)
                    taskname = "sqoop_{}_{}".format(dbname, tabname)
                    ssh_sqoop = getsshoperator(taskname, False, command)
                    command='{} {} {} {} true'.format(scriptpaths["baseval"], dbname, tabname, srctoland[dbname])
                    taskname = "validation_{}_{}".format(dbname, tabname)
                    ssh_valid = getsshoperator(taskname, False, command)
                    command="{} {} {} {} {} {}".format(scriptpaths["hiveload"], tabname , batchid,  'dml', land2stg[dbname], 'stage')
                    taskname = "load_stage_{}_{}".format(dbname, tabname)
                    ssh_stage = getsshoperator(taskname, False, command)
            ...
            with TaskGroup(group_id="{}_depstagetab".format(stagegrp)) as run_depstage:
                ...
                for stagedeptab in sqoopjobs[stagegrp]["depstage"]:
                    ...
                    command="{} {} {} {} {} {}".format(scriptpaths["hiveload"], tabname , batchid,  'dml', dbname, 'stage')
                    taskname = "load_depstage_{}_{}".format(dbname, tabname)
                    ssh_fact = getsshoperator(taskname, False, command)
            ...
            with TaskGroup(group_id="{}_FactLoad".format(stagegrp)) as run_fact:
                ...
                for table in sqoopjobs[stagegrp]["facttabs"]:
                    ...
                    command="{} {} {} {} {} {}".format(scriptpaths["hiveload"], table , batchid,  'dml', factdb, 'fact')
